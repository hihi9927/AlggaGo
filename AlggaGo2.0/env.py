...
class AlggaGoEnv(gym.Env):
    ...
    def set_forced_strategy(self, strategy: int | None):
            # strategy: None, 0, 1만 허용
            if strategy is not None and strategy not in (0, 1):
                raise ValueError("forced_strategy must be None, 0, or 1")
    ...
    def step(self, action):
        ...
        if len(opponent_stones) < 2:
            ...
            if self.forced_strategy is not None:
                strategy_choice = int(self.forced_strategy)
            else:

...
class AlggaGoEnv(gym.Env):
    ...
    def __init__(self, reward_fn=None):
        super().__init__()
        self.reward_fn = reward_fn  # reward_fn(player, prev_black, prev_white, curr_black, curr_white, info) -> float
    ...
    def step(self, action):
        ...
        if len(action) == 3:
            action = np.array([1.0, -1.0, action[0], action[1], action[2]], dtype=np.float32)
        ...
        if current_black == 0 and current_white > 0:
            ...
            terminated = True; info['winner'] = 'black'

        info['black_removed']      = black_removed
        info['white_removed']      = white_removed
        info['is_regular_success'] = (strategy_choice == 0 and (white_removed if self.current_player == "black" else black_removed) > 0)
        info['is_split_success']   = (strategy_choice == 1 and wedge_reward > 0)
        info['wedge_reward']       = wedge_reward

        prev_black = self.initial_black_count
        prev_white = self.initial_white_count
        if self.reward_fn is not None:
            reward = self.reward_fn(
                self.current_player, prev_black, prev_white,
                current_black, current_white, info
            )
    ...
    def render(self, screen=None):  # type: ignore[override]
        if screen is None:
            return

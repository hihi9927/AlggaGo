...
# 드래그 관련 상수
MAX_DRAG_LENGTH = 100    # (일반) 최대 허용 드래그 거리
FORCE_MULTIPLIER = 20     # (임펄스 배율) 드래그 길이에 곱해줄 배율
...
#     from alggago.physics import create_stone  # 지연 로딩 (또는 top-level에서 import 하세요)
#     for shape in stones:
...
def reset_stones(space, stones):
    """모드 1, 2, 3의 고정 배치 함수"""
    from alggago.physics import create_stone  # 지연 로딩 (또는 top-level import)
...
def reset_stones_beginner(space, stones):
    ...
    from alggago.physics import create_stone
